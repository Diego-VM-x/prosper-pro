...
## Hitos Completados
- ✅ **Sistema Multi-Moneda - Fase 2: Integración DolarAPI (19/05/2026)**: Limpieza de monedas a únicamente `BS` y `USD`. Integración del endpoint `app/api/exchange-rates/route.ts` que consulta a `ve.dolarapi.com` (tasa Paralela y Oficial). Actualización de `CurrencyContext` para obtener las tasas desde esta API de forma automática en tiempo real. Configuración rediseñada para mostrar la tasa de DolarAPI con un fallback para un override manual del USD.
