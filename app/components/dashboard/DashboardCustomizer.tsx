    ...
    <div className="dashboard-customizer-page">
      ...
      <div className="dashboard-customizer-header">
        <div className="dashboard-customizer-header-left">
          ...
          <h1 className="dashboard-customizer-title">{t('customize.title', )}</h1>
        </div>
        <button
          className="btn btn-primary btn-sm dashboard-back-btn-mobile"
          onClick={() => router.push('/')}
        >
          <ArrowRight size={14} /> {t('common:buttons.back', )}
        </button>
      </div>
        ...
        <div className="customizer-body">
              ...
              {newCatForm && (
                <div className="customizer-form">
                  <input
                    ...
                    placeholder={t('customize.categoryName', )}
                    value={newCatName}
                    onChange={(e) => setNewCatName(e.target.value)}
                  />
                  <div className="customizer-form-row">
                    <span className="form-label">{t('customize.icon', )}</span>
                ...
                {sortedCategories.map((cat, idx) => (
                  <div key={cat.id} className="customizer-list-item">
                    ...
                    <div className="customizer-list-actions">
                      <button className="customizer-list-btn" onClick={() => moveCategory(cat.id, 'up')} disabled={idx === 0} title={t('customize.moveUp')}>
                        <ArrowUp size={14} />
                      </button>
                      <button className="customizer-list-btn" onClick={() => moveCategory(cat.id, 'down')} disabled={idx === sortedCategories.length - 1} title={t('customize.moveDown')}>
                        <ArrowDown size={14} />
                      </button>
                      <button className="customizer-list-btn" onClick={() => { setEditingCat(cat.id); setEditCatName(cat.name); }} title={t('common:buttons.edit')}>
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (hasWidgets) {
                            if (confirm(t('customize.deleteCategoryConfirm', ))) {
          ...
          {activeTab === 'catalog' && (
            <div className="customizer-tab-content">
              <h3>{t('customize.catalog', )}</h3>
              <p className="customizer-desc">{t('customize.catalogDesc', )}</p>
                ...
                {WIDGET_CATALOG.map(meta => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="widget-catalog-name">{t(meta.label)}</span>
                      <span className="widget-catalog-desc">{t(meta.description)}</span>
                      {isAdded && (
                        <span className="widget-catalog-badge">{t('customize.added', )}</span>
                      )}
                    </button>
                  );
                })}
                    ...
                    >
                      ...
                      <div className="customizer-widget-item-controls">
                        ...
                        <CustomSelect
                          ...
                          options={[
                            { value: UNGROUPED_CATEGORY_ID, label: t('customize.ungrouped', ) },
                        ...
                        {!isFixedWidget(widget) && (
                          <>
                            <button className="customizer-list-btn" onClick={() => moveWidget(widget.id, 'up')} disabled={idx === 0} title={t('customize.moveUp')}>
                              <ArrowUp size={14} />
                            </button>
                            <button className="customizer-list-btn" onClick={() => moveWidget(widget.id, 'down')} disabled={idx === ungroupedWidgets.length - 1} title={t('customize.moveDown')}>
                              <ArrowDown size={14} />
                            </button>
                            <button
                              className="customizer-list-btn danger"
                              onClick={() => {
                                if (confirm(t('customize.removeWidgetConfirm', ))) {
                          ...
                          <div className="customizer-widget-item-controls">
                            ...
                            {!isFixedWidget(widget) && (
                              <>
                                ...
                                <button className="customizer-list-btn" onClick={() => moveWidget(widget.id, 'down')} disabled={idx === catWidgets.length - 1} title={t('customize.moveDown')}>
                                  <ArrowDown size={14} />
                                </button>
                                <button
                                  className="customizer-list-btn danger"
          ...
          <div className="modal-overlay" onClick={() => setAddingWidget(null)}>
            <div className="modal-content" onClick={(e) => e.stopPropagation()} style={{ maxWidth: 420 }}>
              <div className="modal-header">
                <h3 className="modal-title">{t('customize.addWidget', )}</h3>
              ...
              <div className="modal-body">
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
                  ...
                  <span style={{ fontWeight: 600 }}>{t(getWidgetMeta(addingWidget).label)}</span>
                </div>
                <label className="form-label">{t('customize.category', )}</label>
